...
file.save(os.path.join(app.config['UPLOAD_FOLDER'], image))
      ...
      age = form.age.data
      sex = form.sex.data
      addinfo = User(firstname, lastname, age, sex, image)
...
def profileid(userid):
  ...
  if request.method == "POST":
    ...
    return jsonify(user = user.userid, profile_add_on = date, firstname = user.firstname, lastname = user.lastname, age = user.age, sex = user.sex)
